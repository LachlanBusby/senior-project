...
def trees2stmts(trees):
	""" returns two lists of line text and type for each statement """
	stmts = []
	types = []
	for t in trees:
		nodes = t.getPreOrderTraversal()
		for n in nodes:
			continue if not n.isStmt()
			stmts.append(n.getLine())
			types.append(n.getStmtType())
	return stmts,types
...
### substitution strings for literal values ###
VAR_SUB = "___VAR___"
...
# 		sub_yield, t_subs = sub_tokens(orig_yield)
# 		t.setWords(sub_yield)
# 		subs.append(t_subs)
# 	return trees, subs

# def postprocess_trees(trees, subs):
# 	for idx, t in enumerate(trees):
# 		sub_yield = t.getYield()
# 		orig_yield = restore_tokens(sub_yield, subs[idx])
